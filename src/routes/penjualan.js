...
router.post('/', (req, res) => {
  const { tgl, shift = '', detail = {}, total_resep, total_nominal, ket = '' } = req.body
  ...
  const item = { id: Date.now(), tgl, shift, detail, total_resep: totalResep, total_nominal: totalNominal, ket, created_at: new Date().toISOString() }
